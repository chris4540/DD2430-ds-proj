...
if __name__ == '__main__':
    ...
    with open(file, 'r') as f:
        ...
        for i, line in enumerate(f.readlines()):
            if i > 1:
                images_partition.append(
                    [word.strip() for word in line.split(' ') if len(word) > 0])
    # ---------------------------------------------------------------------------------------------------
    ############################################################
    ...
    partition_df = pd.DataFrame(
        images_partition, columns=['images', 'dataset'])

    # category_label: str -> int
    img_df = img_df.astype({'category_label': 'int32'})

    data_df = img_df.merge(partition_df, on='images')
    data_df['category'] = data_df['category_label'].apply(
        lambda x: categories[x - 1])
    ...
    filtered_df = data_df.groupby(col).filter(
        lambda x: len(x) > cfg.min_nb_cnt)
    ...
    nclasses = sampled_df['category'].nunique()
    print("# of classes: ", nclasses)
    print("--------")
    print("Dataset split:")
    ...
    print("--------------------------------------")
    # Rebuild class index
    classes = sampled_df['category'].unique()
    classes.sort()
    # build a mapping from category to index
    cat_to_idx = {k: i for i, k in enumerate(classes)}
    # map category_label to label (index)
